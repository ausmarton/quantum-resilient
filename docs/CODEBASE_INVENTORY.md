...
- `build_final_report.py` - Generate report-ready reports
...
- `validate_dissertation_data.sh` - Validate experiment data completeness for analysis
...
- `extract_analysis_tables.py` - Extract analysis tables (LaTeX/CSV from results)
...
**Purpose**: Tools for generating report-ready research artifacts.
...
- `methodology-measurement.md` - Methodology (measurement and analysis)
...
### Other Files
- `feedback-draft-1` - Feedback draft (if present)
...
**Purpose**: Aggregated final results (tables, figures, reports).
